// =============================================================================
// FIXED: Voice format "Product name size type X count" (not "X product name")
...
// Generate optimized spoken response - "Product name size type X count"
function generateSpoken(action, data, parsed, parsed2) {
  if (action === 'next_item') {
    ...
    if (data.count === 2 && data.item2_product_name) {
      ...
      if (parsed2.type) {
        var type2Lower = parsed2.type.toLowerCase();
        var name2Lower = parsed2.name.toLowerCase();
        if (name2Lower.indexOf(type2Lower) === -1) {
