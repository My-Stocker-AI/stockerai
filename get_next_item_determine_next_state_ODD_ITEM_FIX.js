// FIXED: Handle boundary condition when count=2 but only 1 item remaining
// Ensures EVERY item gets announced regardless of odd/even count
...
var requestedCount = input.count || 1;
...
// ===== NEW: Calculate actual available items =====
var availableItems = 0;
if (pickDirection === 'reverse') {
  // Reverse: count items with sequence < currentItemIndex
  for (var i = 0; i < items.length; i++) {
    if (items[i].sequence < currentItemIndex) {
      availableItems++;
    }
  }
} else {
  // Forward: count items with sequence > currentItemIndex
  for (var i = 0; i < items.length; i++) {
    if (items[i].sequence > currentItemIndex) {
      availableItems++;
    }
...
// ===== FIX: Adjust count to what's actually available =====
var count = requestedCount;
if (count > availableItems) {
  count = availableItems;  // If requesting 2 but only 1 left, pick 1
}

// Find next item (advance by adjusted count)
var nextItem = null;
var targetIndex;
if (pickDirection === 'reverse') {
  targetIndex = currentItemIndex - count;
} else {
  targetIndex = currentItemIndex + count;
}
...
// If count=2, also get item2 (only if we actually picked 2)
var item2 = null;
if (nextItem && count === 2) {
  var item2Index;
  if (pickDirection === 'reverse') {
...
if (nextItem) {
  ...
  return [{
    json: {
      ...
      count: count  // Return actual count picked (might be 1 even if 2 requested)
    }
  }];
}
