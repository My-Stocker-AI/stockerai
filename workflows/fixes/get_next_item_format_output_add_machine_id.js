...
// FIX: Add machine_id to next_item response for frontend state sync
// Date: 2026-02-08
// =============================================================================
...
function generateSpoken(action, data, parsed, parsed2) {
 ...
 if (action === 'next_machine') {
      ...
      if (returning) {
        return data.completed_machine + ' complete. Going back to skipped machine ' + data.next_machine + ' at ' + data.next_location + '. Top or bottom?';
      } else {
...
if (data.action === 'next_item') {
  ...
  output.machine_id = data.machine_id;        // ← FIX: Added for frontend state sync
