// CONCURRENT FIX: Add version check to prevent race conditions
// Problem: Two "next" commands read same state, both increment, causing skip/premature completion
// Solution: Check that current_item_index hasn't changed since we read it
...
if (nextMachine) {
  // FIX: Set correct starting index for reverse vs forward mode
  var startingIndex;
  if (pickDirection === 'reverse') {
