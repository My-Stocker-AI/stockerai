// COMPLETE FIX: All critical bugs including inventory display
//
// FIXES:
// 1. Items remaining calculation off by 1 in reverse mode (line 74)
// 2. Next machine startingIndex uses wrong items.length (lines 126, 156)
// 3. Inventory fields missing from output (lines 87-90) ← NEW FIX
//
...
if (nextItem) {
  ...
  return [{
    json: {
      ...
      // FIX: Add inventory fields (was causing 0/0 display)
...
// NOTE: Reverse pick direction only affects ITEM order, not machine order
// Always go through machines sequentially: 1 → 2 → 3 → 4 → 5 → 6 → 7
