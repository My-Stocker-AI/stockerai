// PHASE 2: Use machines.completed_items for progress tracking
//
// CRITICAL CHANGE: Replace derived item counts with database completed_items
//
// FIXES:
// - Bug #2: Machine 2 shows "2/5" (now uses per-machine completed_items)
// - Bug #3: Machine finishes after 3 items (completion check: completed_items >= total_items)
//
// CONTRACT COMPLIANCE:
// - machines.total_items = IMMUTABLE (never changes)
// - machines.completed_items = MUTABLE (0 → total_items, per-machine, isolated)
// - items_remaining = total_items - completed_items (per-machine calculation)
//
...
// CRITICAL: Use completed_items from database (PRIMARY source of truth)
...
// Check if machine complete BEFORE trying to get next item
// CONTRACT: Machine complete when completed_items >= total_items
if (completedItems >= totalItems) {
  // Machine already complete - should not happen, but handle gracefully
  // This means we need to move to next machine
...
// Get next item based on current index
var nextItem = null;
...
if (nextItem) {
  ...
  // PHASE 2: Calculate items_remaining from completed_items (not derived)
  // After this item is picked, completed_items will be incremented by HTTP node
...
// No more items on machine - check completion with completed_items
