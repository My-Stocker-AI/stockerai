// REVERSE MODE FIX: Machines are ALWAYS visited in forward sequence order
// "Reverse" only affects item picking order WITHIN each machine, not machine order
...
// FIX: No more items on machine - find next machine IN FORWARD ORDER
// Machines are ALWAYS visited forward (1→2→3), regardless of pick_direction
...
if (nextMachine) {
  // Set correct starting index based on pick direction
  var startingIndex;
  if (pickDirection === 'reverse') {
