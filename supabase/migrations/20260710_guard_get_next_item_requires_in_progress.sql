-- Guard get_next_item_and_increment: the count may only advance a machine that is
-- actually being stocked (status='in_progress'). An out-of-order get-next-item fired
-- BEFORE start-machine advanced a still-'pending' machine's completed_items and left a
-- phantom count that skipped items (Davy's 2026-07-10 North walk). This CREATE OR REPLACE
-- adds the in_progress guard just after the current-machine fetch.
-- Applied to prod 2026-07-10 via management API; captured here for version control.
