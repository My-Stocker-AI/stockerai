-- ROLLBACK: Performance fix broke machine transitions
-- Reverting to original query that joins ALL items from ALL machines
...
BEGIN
  ...
  LEFT JOIN items i ON i.machine_id = m.id  -- ROLLBACK: Join ALL items again
...
COMMENT ON FUNCTION get_next_item_data IS 'Reverted to original - joins all items from all machines (performance fix broke transitions)';
