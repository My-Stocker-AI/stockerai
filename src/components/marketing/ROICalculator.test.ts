...
test('five drivers use picking hours rather than the entire workday', () => {
  show();
...
test('zero picking time has no labor benefit and still includes subscription', () => {
  show();
  fireEvent.change(screen.getByLabelText('Picking hours per driver per workday'), {target:{value:'0'}});
...
test('changing workdays changes annualized savings; invalid negative hours cannot create savings', () => {
  ...
  fireEvent.change(screen.getByLabelText('Picking hours per driver per workday'), {target:{value:'-2'}});
  expect(screen.getByText('$0.00')).toBeTruthy();
});
